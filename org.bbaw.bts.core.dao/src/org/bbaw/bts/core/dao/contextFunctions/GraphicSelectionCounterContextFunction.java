...
public class GraphicSelectionCounterContextFunction extends ContextFunction {
	...
	public Object compute(IEclipseContext context) {
		...
		GraphicSelectionCounterDao dao;
		try {
			dao = loadDaoFactory(context).createDao(
					GraphicSelectionCounterDao.class, context);
		...
		context.set(GraphicSelectionCounterDao.class, dao);

		return dao;
	}

	private DAOFactory loadDaoFactory(IEclipseContext context)
		...
		for (IConfigurationElement e : config) {
			...
			if (o instanceof DAOFactory
					&& (daoFactoryName == null || daoFactoryName
							.equals(((DAOFactory) o).getFactoryName()))) {
				return (DAOFactory) o;
			}
		}
		throw new BTSDBException(
				"No DaoFactory found for DBLeaseDao and factory name: "
						+ daoFactoryName);
